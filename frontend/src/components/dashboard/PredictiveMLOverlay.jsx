...
export default function PredictiveMLOverlay() {
  ...
  // Dismiss handler — sets isSimulating false in the store
  const { setSimulating } = useAppStore();
  ...
  return (
    <div 
      ref={overlayRef}
      className="absolute top-1/4 left-1/2 -translate-x-1/2 bg-slate-900/80 backdrop-blur-3xl border border-primary/40 p-6 rounded-2xl shadow-[0_0_40px_rgba(59,130,246,0.4)] z-50 flex items-center space-x-6 opacity-0"
    >
      {/* Dismiss button */}
      <button
      ...
      <div className="relative z-10 pr-4">
        ...
        <p className="text-slate-400 mt-1">
          Adjusting ETAs and routing for <span className="text-primary font-bold">{trains.length}</span> active consists...
        </p>
      </div>
    </div>
  );
